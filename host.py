# -*- coding: utf-8 -*-
#Toxic  Host Bot — Credit: @botsarefather
import telebot
import subprocess
import os
import zipfile
...
# --- Configuration ---
TOKEN = '8856643374:AAEZJ1q4OyyKrOeRZACjw9BHBhuVzLzhnVE'
OWNER_ID = 6325764594
ADMIN_ID  = 6325764594
YOUR_USERNAME = '@zixplays'
UPDATE_CHANNEL = '@zixplays'
FORCE_JOIN_CHANNEL = '@zix_store'   # Change to your channel username (with @)
...
def is_member(user_id: int) -> bool:
    """Return True if the user is a member of FORCE_JOIN_CHANNEL."""
    ...
    try:
        ...
        return True   # fail-open to avoid locking out everyone on API error
...
def require_join(func):
    ...
    def wrapper(message, *args, **kwargs):
        ...
        if uid and not is_member(uid):
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton(
                "✅ Join Channel", url=f"https://t.me/{FORCE_JOIN_CHANNEL.lstrip('@')}"))
            markup.add(types.InlineKeyboardButton("🔄 I Joined — Verify", callback_data='verify_join'))
            bot.reply_to(message,
                f"⚠️ *You must join our channel to use {BOT_NAME}!*\n\n"
...
def _logic_send_welcome(message):
    ...
    if not is_member(user_id):
        ...
        bot.send_message(chat_id,
            ...
            f"⚠️ You must join our channel first to continue.\n\n"
...
def handle_document(message):
    ...
    if not is_member(user_id):
        ...
        markup.add(types.InlineKeyboardButton("✅ Join Channel",
            url=f"https://t.me/{FORCE_JOIN_CHANNEL.lstrip('@')}"))
        markup.add(types.InlineKeyboardButton("🔄 I Joined — Verify", callback_data='verify_join'))
        bot.reply_to(message, f"⚠️ Join {FORCE_JOIN_CHANNEL} first!",
    ...
    if data == 'verify_join':
        if is_member(user_id):
            ...
            bot.answer_callback_query(call.id,
                f"❌ You haven't joined {FORCE_JOIN_CHANNEL} yet!", show_alert=True)
        return
